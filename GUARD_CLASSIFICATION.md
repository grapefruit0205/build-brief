...
| Exact one-use runner token, action, request, root and state binding | mutation claims in `hooks/click_mutation.py`; managed-service claims in `hooks/click_service.py`; observation claims in `hooks/click_observation.py`; remaining verification claims in `hooks/click_gate.py` | Prevents replay, request substitution and cross-state execution |
