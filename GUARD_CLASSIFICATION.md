...
| Protected Git-tree, environment and executable fingerprints for argv verification; planned-reuse workspace revalidation before the one-use claim; verification-time mutation detection | receipt helpers and runners in `hooks/click_verification.py` | Preserves the identity of the code and environment actually checked or authoritatively reused by Click's argv runner |
...
| Committed verification-name resolution, exact argv selection, and pre-execution catalog revalidation | schema v2 in `hooks/click_evidence_shards.py`; request expansion and runner claim in `hooks/click_verification.py` | Lets a stable repository id select one exact check definition without treating the name as approval, evidence, command equivalence, or reuse authority |
...
| Content-free reuse diagnostics linked to canonical decisions and witnessed batch outcomes | `hooks/click_reuse_diagnostics.py`; comparison capture in `hooks/click_verification.py`; read-only projection in `hooks/click_dashboard_projection.py` | Explanatory telemetry only — toggling it cannot alter a plan decision, runner batch, receipt authority, or Guarded approval |
