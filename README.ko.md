...
Click은 명시적으로 선택하는 Codex 플러그인입니다. `@Click`을 멘션하고 자연어로 원하는 소프트웨어를 말하면 관련 저장소를 읽어 완전한 개발자용 실행 계약으로 번역하고, 같은 내용을 쉬운 말로 설명하고, 적절한 최종 검증 규모를 추천한 뒤 코딩 전에 한 번만 승인을 요청합니다.
