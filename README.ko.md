...
Click은 명시적으로 선택하는 Codex 플러그인입니다. `@Click`을 멘션하고 자연어로 원하는 소프트웨어를 말하면 관련 저장소를 읽어 축약된 개발자용 실행 계약으로 번역하고, 같은 내용을 쉬운 말로 설명하고, 적절한 최종 검증 규모를 선택한 뒤 코딩 전에 한 번만 승인을 요청합니다.
...
저장소의 결정적 테스트는 명시 호출 전용, 미호출 fail-open, opt-out, 안전한 읽기 명령, 축약 계약 완전성, 조건부 build 제약, 검증 규모, stage/pass digest 동일성, One-shot 계약 잠금, digest 전용 상태, 저장소 정책을 다룹니다. 공개 CI는 Linux·macOS·Windows에서 실행됩니다.
