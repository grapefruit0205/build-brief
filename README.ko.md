...
Click은 명시적으로 선택하는 Codex 플러그인입니다. `@Click`을 멘션하고 자연어로 원하는 소프트웨어를 말하면 관련 저장소를 읽어 축약된 개발자용 실행 계약으로 번역하고, 같은 내용을 쉬운 말로 설명하고, 적절한 최종 검증 규모를 선택한 뒤 코딩 전에 한 번만 승인을 요청합니다. 승인 뒤에는 Hook이 관찰 가능한 탐색·재계획 루프를 막고, 구현이 끝나면 자동 예산 안에서 최종 검증 명령을 계량하고 실행합니다.
...
4. 사용자가 한 번 승인합니다.
5. Click이 계약 안에서 One-shot으로 구현하는 동안 Hook이 관찰 가능한 반복을 막고, 예산을 적용해 마지막 검증 묶음을 한 번 실행합니다.
...
flowchart LR
    ...
    E --> G["관찰 가능한 반복<br/>Hook 차단"]
    G --> F["예산이 적용된<br/>마지막 검증 한 묶음"]
    D -->|제안 변경| B
```
...
Hook은 계약 digest와 본문을 포함하지 않는 관찰·검증 상태 정보를 대상 저장소 밖에 기록하며 계약 본문, 읽기·검색 명령, 출력은 저장하지 않습니다. 승인 뒤 전달되는 계약은 JSON을 정규화했을 때 stage한 계약과 정확히 같아야 하며, 다른 계약이면 거부합니다.
...
Hook이 강제하는 것은 계약 형식·순서·stage와 pass 계약의 동일성, 관찰 가능한 anti-loop 규칙, 그리고 화면에 보이는 검증 명령에 대한 결정적 예산 정책입니다. 아키텍처가 옳은지, 사람이 실제로 승인했는지, 모든 코드가 의미상 계약과 일치하는지, 사용자 정의 스크립트 내부에서 실제로 얼마나 많은 검증이 실행되는지는 증명하지 못합니다.

## 실행 루프 없이 구현

승인 뒤에는 축약 계약 하나가 계속 유일한 기준입니다. 새 계획을 만들거나 모든 행동을 phase와 step으로 다시 분류하지 않습니다.

Hook은 눈에 보이는 네 가지 낭비를 결정적으로 처리합니다.

- 승인 뒤의 읽기 전용 Bash 명령은 로컬 관찰 러너가 실행합니다. 상태에는 정규화한 명령 digest, 변경 revision, 종료 코드, 출력 크기, 상태만 기록하고 명령과 출력 본문은 저장하지 않습니다. 범위 안의 코드 변경이 없는데 성공한 동일 읽기·검색을 다시 실행하면 차단합니다.
- 실패했거나 출력이 48,000바이트를 넘은 관찰은 변경 없이 한 번 재시도할 수 있습니다. 그래도 실패하거나 불완전하면 다음 명령은 범위를 좁히거나 실질적으로 달라야 합니다.
- 일치하는 `update_plan` 호출과 승인된 계약을 다시 stage하거나 교체하려는 모든 시도를 거부합니다. 실제 결과나 경계를 바꿔야 한다면 사용자에게 돌아가야 합니다.
- 승인 뒤 루트 기준 `rg --files`, `find .`, 재귀적인 루트 `ls`/`Get-ChildItem`, `tree`, `git ls-files`, 범위를 지정하지 않은 재귀 `git ls-tree` 같은 저장소 전체 목록 재탐색을 거부합니다. 경로를 좁힌 목록 확인과 구체적인 내용 검색은 허용합니다.

아래에서 설명하는 것처럼 `click-gate verify` 밖에서 실행하는 광범위 검증도 거부합니다. 코드가 변경되면 성공한 관찰 기록을 비워 바뀐 파일을 다시 읽을 수 있게 합니다.

이 규칙은 Hook matcher에 보이는 도구 호출만 다룹니다. 모델의 숨은 추론, 설명 문장으로만 작성한 계획, matcher 밖의 MCP·connector 읽기, 자연어 경계를 모든 코드가 의미상 지켰는지는 볼 수 없습니다. 도구 루프를 줄이는 장치이지 추론 토큰 상한이나 보안 샌드박스가 아닙니다.
...
저장소의 결정적 테스트는 명시 호출 전용, 미호출 fail-open, opt-out, 안전한 읽기 명령, 축약 계약 완전성, 조건부 build 제약, 동일 읽기 차단과 제한된 실패 재시도, 전체 목록 재탐색과 재계획 거부, 검증 규모와 단위 상한, stage/pass digest 동일성, One-shot 계약 잠금, 최종 검증 실행·재시도 상태, 본문을 저장하지 않는 Hook 상태, 저장소 정책을 다룹니다. 공개 CI는 Linux·macOS·Windows에서 실행됩니다.
