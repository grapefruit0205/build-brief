...
A committed [Evidence Shards map](skills/click/references/evidence-shards-v1.md) schema v2 can also define stable verification names. For example, `click-gate verify '{"version":2,"workdir":"/absolute/repository","names":["auth-unit"]}'` resolves `auth-unit` to its committed exact argv and then uses the ordinary canonical plan and one-use runner. Raw argv requests remain supported; names do not normalize commands, grant approval, or guarantee reuse after code changes. A broad named definition may be split into independent children, retaining a passed sibling after another fails; invalid or edited maps fail closed.
...
Reuse Diagnostics is read-only and on by default. Use `click-gate diagnostics on`, `off`, or `status`. It records the prior-success state, the authoritative decision and primary reason, every already-computed binding mismatch, and explicit `not-evaluated` checks, then joins that frozen plan data to witnessed execution outcomes. Turning it off never changes reuse authority, runner contents, Guarded approval, or receipts. The dashboard shows the most common and highest observed-cost blockers without calling a conservative rerun unnecessary. Cause times may overlap when one verification group has several blockers, so only the separately displayed deduplicated total may be read as a total.
