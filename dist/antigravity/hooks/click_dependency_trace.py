#!/usr/bin/env python3
"""Linux-only, non-authoritative dependency tracing for argv verification.

This leaf executes one already-authorized command and returns its exit status
beside a bounded Shadow Observer v1 record. Shadow data is never converted to
the runtime dependency observation used by evidence reuse.
"""
...
if __package__:
    from . import click_dependency_cache, click_process
...
def combine_records(
    ...
    *,
    ...
    valid = [
        ...
        if click_dependency_cache.shadow_observer_record_is_valid(record)
        and record["binding"]
        == {
            "evidence_key": evidence_key,
            "check_digest": check_digest,
    ...
    if len(valid) != len(records) or not valid:
        return None
    backends = {
        tuple(sorted(record["backend"].items()))
        for record in valid
        if isinstance(record.get("backend"), dict)
    }
    if len(backends) > 1:
        return None
    backend = dict(next(iter(backends))) if backends else None
    observed = [record for record in valid if record["status"] != "unavailable"]
    duration = 0
    overhead = 0
    for record in valid:
        duration = _bounded_add(duration, int(record["command_duration_ms"]))
        overhead = _bounded_add(overhead, int(record["observer_overhead_ms"]))
    if not observed:
        return _record(
            evidence_key=evidence_key,
            check_digest=check_digest,
    ...
    status = (
        "failed"
        if any(record["status"] == "failed" for record in observed)
        else "partial"
        if unavailable_count
        or unexecuted_checks
        or any(record["status"] == "partial" for record in observed)
        else "complete"
    )
    ...
    for record in observed:
        unresolved = _bounded_add(
            unresolved, int(record["unresolved_event_count"])
        )
        external = _bounded_add(
            external, int(record["external_input_count"])
        )
        children = _bounded_add(children, int(record["child_process_count"]))
        inputs.extend(record["inputs"])
    process_tree_complete = bool(
        status == "complete"
        and all(record["process_tree_complete"] for record in observed)
    )
    return _record(
        evidence_key=evidence_key,
        check_digest=check_digest,
...
def state_is_valid(value: Any) -> bool:
    ...
    if (
        ...
        or len(records) > MAX_SHADOW_STATE_RECORDS
    ):
        return False
    return all(
        isinstance(key, str)
        and _DIGEST.fullmatch(key) is not None
        and click_dependency_cache.shadow_observer_record_is_valid(record)
        and record["binding"]["evidence_key"] == key
...
def store_records(
    ...
    for key, record in sorted(records.items()):
        if (
            len(current) >= MAX_SHADOW_STATE_RECORDS
        ...
        if (
            ...
            or not click_dependency_cache.shadow_observer_record_is_valid(record)
            or record["binding"]["evidence_key"] != key
        ):
            continue
        current[key] = record
        stored += 1
...
def advisory(record: Any) -> str:
    ...
    return (
        f"[Click shadow] {record['status']}: inputs={len(record['inputs'])} "
        f"external={record['external_input_count']} "
        f"unresolved={record['unresolved_event_count']} "
        f"overhead={record['observer_overhead_ms']}ms"
    )
