...
def _runner_arguments(arguments: list[str]) -> tuple[list[str], str]:
    ...
    if not arguments or arguments[0] != "--state-root":
        return arguments, ""
    if len(arguments) < 3 or arguments[2] not in STATEFUL_RUNNER_ACTIONS:
        return [], "Click runner state-root binding is malformed."
    state_root = Path(arguments[1])
    if not state_root.is_absolute() or state_root.name != "gate-state":
...
def main() -> int:
    arguments, runner_error = _runner_arguments(sys.argv[1:])
    if runner_error:
