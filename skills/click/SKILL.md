...
- `verification`: one `quick`, `focused`, or `full` scale and observable `done_when` conditions, each assigned one cheapest sufficient primary evidence source, plus `intermediate_gate` only for a real irreversible boundary;
...
Before staging, assign every `done_when` condition exactly one cheapest sufficient primary evidence source. One source may cover several conditions. Prefer current evidence that remains valid for the final revision, then the narrowest adequate automated or static check; use browser, manual, hosted, or long-running evidence only for behavior cheaper sources cannot prove. When Browser is the source, name it explicitly after `primary evidence:` (or the localized equivalent) so the Hook can authorize and meter it. Do not assign both automated and interactive proof to the same condition, enumerate equivalent permutations, or wait through a timed end-to-end flow when a deterministic state transition or representative scenario proves it.
