...
8. **Bound Browser evidence.** The canonical Browser MCP tool is denied during an approved contract unless a `done_when` condition explicitly assigns Browser after `primary evidence:` or its localized equivalent. One assigned representative session gets three serial calls and 90 measured seconds. Tool timeouts over 30 seconds, explicit waits over five seconds, unassigned shadow sessions, and replay after completion are blocked. A later mutation resets the Browser ledger.
...
The Skill and semantic grader still decide whether a source is sufficient and whether two different sources duplicate the same semantic condition. The Hook additionally meters the canonical Browser MCP path and local argv checks, but unmatched connectors remain outside that observable boundary. Do not exploit an unmatched path to create a shadow verification suite.
