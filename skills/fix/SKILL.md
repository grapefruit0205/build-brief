...
Before staging, read [Click's contract format](../click/references/directive-format.md), [verification profiles](../click/references/verification-profiles.md), and [anti-loop policy](../click/references/anti-loop-policy.md). Produce the same six-area contract: `outcome`, scoped `boundary`, `must_hold`, compact `build`, `verification`, and `plain_language`. Omit optional build semantics, order, and an intermediate gate unless the defect genuinely requires them.
