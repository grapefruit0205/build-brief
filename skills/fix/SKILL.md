...
Choose `quick`, `focused`, or `full`; approval of the repair contract approves that scale and its automatically enforced unit ceiling. Declare every source once in `verification.evidence` with a unique id, a `kind` of `argv`, `browser`, `hosted`, `manual`, or `existing`, and a short description. Each `done_when` condition references exactly one id through `primary_evidence`; one id may cover several conditions. Prefer current valid evidence and narrow automated checks; use browser, manual, hosted, or long-running evidence only when cheaper sources cannot prove the condition. Prefer one final completion batch. Add an intermediate gate only for an irreversible or externally consequential boundary.
