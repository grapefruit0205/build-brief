...
Use Click's shared [contract format and approval lifecycle](../click/references/directive-format.md) and [verification profiles](../click/references/verification-profiles.md). Stage the repair JSON once, keep `plain_language` digest-bound, capture its `contract_id` from `CLICK_CONTRACT_ID`, and project the four human sections once: Goal, Changes, Unchanged, and Completion checks. Keep JSON as optional Technical contract details, ask once, and stop without editing. Only after explicit approval in a later user turn, pass that id—never resend the JSON.
